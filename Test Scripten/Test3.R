...
x<-tapply(breaks,list(tension,wool), Mean2)
#class(x) <- c(class(x), "xtabs")

#
APA2(x)
with(DF, tapply(SOC, list(BMI , Edu , Sex), Mean2))
...
APA2(~BMI+ PSD + SOC + IQ , DF, test=T, caption="Tabelle 2")
APA2(lm(BMI~ PSD + SOC + IQ , DF))
...
APA2(lm(BMI~ PSD + SOC + IQ , DF) , pvalues=TRUE )
...
APA2(list(model.1, model.2,model.3 ),caption="Abbildung 4", custom.model.names=c("Model A","B","C")
    , single.row=F
...
#library(reshape)
#APA2(~BMI+ PSD + SOC + IQ , DF, fun=function(x) quantile(x,na.rm=T))
APA2(~BMI+ PSD + SOC + IQ , DF, fun=function(x) {x<-quantile(x,na.rm=T); paste(x[2],x[3],x[4],sep="|")     })
#APA2(~ Sex, DF, fun=table )
#APA(~ Edu, DF, fun=table )
#APA2(Edu~Sex  , DF, fun=table )
#APA2(Edu~Sex  , DF, fun=function(x) round(prop.table(table(x))*100))
...
#APA2(Edu~Sex  , DF, fun=Prozent)
APA2(BMI~Sex  , DF, fun=Prozent,  breaks = fivenum(DF$BMI, na.rm = TRUE)[-3],labels = Cs(low, med, hig))





APA2(BMI+ PSD~Sex  , DF, fun=mean2 )
APA2(BMI+ PSD~Sex  , DF, fun=Mean2)

APA2(BMI+ PSD~Sex  , DF,  fun=Median2)



APA2(~BMI+ PSD + SOC + IQ , DF )
...
#Start("html")
APA2(x<-xtabs( ~Edu+Sex  , DF ), margin=2)
